...
// app报告处理完成
+ (BOOL) reportFinish:(NSString*) region :(BOOL) isSuccess;

// 手动模糊匹配后,更新scanResult
+ (NSString*) updateScanMatchMatrix:(NSString*) region :(NSString*) newMatrix NS_SWIFT_NAME(updateScanMatchMatrix(region:newMatrix:));

// 更新scanResult
+ (BOOL) updateScanResult:(NSString*)region :(NSString*)scanResult NS_SWIFT_NAME(updateScanResult(region:scanResult:));
...
+ (NSString*) updateScanMatchWallMapping:(NSString*)region :(NSString*) drawWallId :(NSString*) scanWallId;
